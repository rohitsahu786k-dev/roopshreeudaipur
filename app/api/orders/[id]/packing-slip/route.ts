...
import { generatePackingSlipBuffer } from "@/lib/order-documents";
...
export async function GET(_request: NextRequest, { params }: Context) {
  try {
    ...
    const buffer = generatePackingSlipBuffer(order, (settings ?? undefined) as any);
    ...
    const message = error instanceof Error ? error.message : "Unable to generate packing slip";
