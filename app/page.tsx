  ...
  return (
    <Link href={`/product/${product.slug}`} className="group block bg-white">
      ...
      <div className="pt-2 text-center">
        ...
        <p className="mt-1 text-[12px] font-bold text-primary">{formatPrice(product.price)}</p>
      </div>
    </Link>
  );
}
  ...
  return (
    <div className="bg-white text-ink">
      ...
      <main className="mx-auto max-w-[1728px] px-4 py-5 lg:px-7">
        ...
        <section className="mt-10 bg-[#eef5fb] px-4 py-8">
          <div className="grid gap-5 lg:grid-cols-[0.55fr_1.45fr] lg:items-center">
            <div className="text-center lg:text-left">
              ...
              <Link href="/shop" className="mt-5 inline-flex bg-primary px-6 py-2 text-[11px] font-bold uppercase text-white">
                Shop Now
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {designerCards.map((card) => (
                <Link key={card.title} href="/shop" className="group block bg-white p-2">
                  <div className="relative aspect-[3/4] overflow-hidden">
                    <Image src={card.image} alt={card.title} fill className="object-cover transition duration-500 group-hover:scale-105" sizes="(min-width: 1024px) 18vw, 50vw" />
                  </div>
